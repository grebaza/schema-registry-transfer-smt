 ...
 *
 * <ul>
 *   <li>you can register a schema and
 *   <li>retrieve a schema by id.
 * </ul>
 *
 * <p>Additionally, server-side mock can be toggled from its default authentication behavior (no
 * authentication) to a variant that requires basic HTTP Authentication using fixed credentials
 * `username:password` by placing a `@Tag(Constants.USE_BASIC_AUTH_SOURCE_TAG)` and/or
 * `@Tag(Constants.USE_BASIC_AUTH_DESTR_TAG)` annotation after @Test annotation of any basic HTTP
 * authentication dependent test code.
 *
 * <p>If you use the TestToplogy of the fluent Kafka Streams test, you don't have to interact with
 * this class at all.
 *
 * <p>Without the test framework, you can use the mock as follows:
 *
 * <pre>
 * <code>
 ...
 *       </pre>
 *
 * <p>To retrieve the url of the schema registry for a Kafka Streams config, please use {@link
 * #getUrl()}
 */
public class SchemaRegistryMock implements BeforeEachCallback, AfterEachCallback {
  ...
  public SchemaRegistryMock(Role role) {
    ...
    this.basicAuthTag =
        (role == Role.SOURCE)
            ? Constants.USE_BASIC_AUTH_SOURCE_TAG
            : Constants.USE_BASIC_AUTH_DEST_TAG;
    ...
    if (context.getTags().contains(this.basicAuthTag)) {
      ...
      this.stubFor =
          (MappingBuilder mappingBuilder) ->
              this.mockSchemaRegistry.stubFor(
                  mappingBuilder.withBasicAuth(userPass[0], userPass[1]));
    } else {
      this.stubFor =
          (MappingBuilder mappingBuilder) -> this.mockSchemaRegistry.stubFor(mappingBuilder);
    }

    this.mockSchemaRegistry.start();
    this.stubFor.apply(
        WireMock.get(WireMock.urlPathMatching(SCHEMA_REGISTRATION_PATTERN))
            .willReturn(WireMock.aResponse().withTransformers(this.listVersionsHandler.getName())));
    this.stubFor.apply(
        WireMock.post(WireMock.urlPathMatching(SCHEMA_REGISTRATION_PATTERN))
            .willReturn(
                WireMock.aResponse().withTransformers(this.autoRegistrationHandler.getName())));
    this.stubFor.apply(
        WireMock.get(WireMock.urlPathMatching(SCHEMA_REGISTRATION_PATTERN + "/(?:latest|\\d+)"))
            .willReturn(WireMock.aResponse().withTransformers(this.getVersionHandler.getName())));
    this.stubFor.apply(
        WireMock.get(WireMock.urlPathMatching(CONFIG_PATTERN))
            .willReturn(WireMock.aResponse().withTransformers(this.getConfigHandler.getName())));
    this.stubFor.apply(
        WireMock.get(WireMock.urlPathMatching(SCHEMA_BY_ID_PATTERN + "\\d+"))
