...
public class TransformTest {
  ...
  private ConnectRecord createRecord(
  ...
  private void configure(
      final String sourceUserInfo, final String destUserInfo, ExplicitAuthType credentialSource) {
  ...
  private void passSimpleMessage() throws IOException {
    ...
    final ConnectRecord record = createRecord(keyOut.toByteArray(), valOut.toByteArray());
  ...
  public void testKeySchemaTransfer() {
    ...
    try {
      ...
      assertEquals(
          1, numSourceVersions, "the source registry subject contains the pre-registered schema");
    ...
    try {
      ...
      ConnectRecord record =
          createRecord(Schema.OPTIONAL_BYTES_SCHEMA, out.toByteArray(), null, null);
      ...
      // In any case, we can still check the key schema was copied, and the destination now has some
      // version
  ...
  public void testKeyValueSchemaTransfer() {
    ...
    try {
      ByteArrayOutputStream keyStream =
          encodeAvroObject(keySchema, sourceKeyId, AVRO_CONTENT_OFFSET);
      ByteArrayOutputStream valueStream =
          encodeAvroObject(valueSchema, sourceValueId, "hello, world");
      ...
      SchemaMetadata valueMetadata =
          destClient.getSchemaMetadata(valueSubject, destValueVersions.get(0));
    ...
    assertNotEquals(
        sourceKeyId, transformedRecordKeySchemaId, "transformed record's key schema id changed");
    assertEquals(
        destinationKeyId,
        transformedRecordKeySchemaId,
        "record key's schema id matches destination id");
    assertArrayEquals(
    ...
    assertNotEquals(
        sourceValueId, transformedRecordValueSchemaId, "transformed record's schema id changed");
    assertEquals(
        destinationValueId,
  ...
  public void testEvolvingValueSchemaTransfer() {
    ...
    try {
      numSourceVersions = sourceClient.getAllVersions(subject).size();
      assertEquals(
          2, numSourceVersions, "the source registry subject contains the pre-registered schema");
    } catch (IOException | RestClientException e) {
      fail(e);
    }
    ...
    try {
      ...
      destVersions = destClient.getAllVersions(subject);
      assertEquals(
          1, destVersions.size(), "the destination registry has been updated with first schema");
